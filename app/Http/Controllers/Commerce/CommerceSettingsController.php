...
class CommerceSettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $qrPath = $merchant->settings['commerce']['payment_qr_path'] ?? null;
        if ($request->hasFile('payment_qr')) {
            $qrPath = $request->file('payment_qr')->store("payment-qr/{$merchant->id}", 'public');
        }
        ...
        $settings = array_merge($merchant->settings ?? [], [
            'commerce' => [
                ...
                // Merchant's OWN payment QR (e.g. their PromptPay) — display only
