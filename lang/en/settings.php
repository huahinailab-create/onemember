<?php

return [
    'title'                      => 'Settings',
    'subtitle'                   => 'Manage your business profile and account preferences.',
    'tab_profile'                => 'Business Profile',
    'tab_preferences'            => 'Business Preferences',
    'tab_account'                => 'Account',
    'tab_security'               => 'Security',
    'tab_data'                   => 'Data',
    'business_name'              => 'Business Name',
    'business_name_ph'           => 'e.g. Happy Coffee Shop',
    'business_name_hint'         => 'Your brand, shown to customers exactly as you type it.',
    // OMEGA-001E — Store Identity & Public URL Foundation
    'store_url'                  => 'Store URL',
    'store_url_hint'             => 'Lowercase letters, numbers, and hyphens only. Used in your storefront link and QR codes.',
    'store_url_checking'         => 'Checking availability…',
    'store_url_available'        => 'Available.',
    'store_url_taken'            => 'That Store URL is already taken.',
    'store_url_reserved'         => 'That Store URL is reserved and can\'t be used.',
    'store_url_confirm_change'   => 'Changing your Store URL changes your public storefront link. Existing marketing links and printed QR codes that point to the old URL will stop working. Continue?',
    'public_store_url'           => 'Public Store',
    'copy'                       => 'Copy',
    'copied'                     => 'Copied!',
    'business_type'              => 'Business Type',
    'business_phone'             => 'Business Phone',
    'website'                    => 'Website',
    'contact_person'             => 'Contact Person',
    'contact_person_ph'          => 'e.g. David Chen',
    'business_email'             => 'Business Email',
    'mobile_number'              => 'Mobile Number',
    'address_line_1'             => 'Address Line 1',
    'address_line_1_ph'          => 'Street address',
    'address_line_2'             => 'Address Line 2',
    'address_line_2_ph'          => 'Unit, floor, building (optional)',
    'city'                       => 'City',
    'state'                      => 'State / Province',
    'postal_code'                => 'Postal Code',
    'country'                    => 'Country',
    'notes'                      => 'Notes',
    'currency'                   => 'Currency',
    'timezone'                   => 'Time Zone',
    'language'                   => 'Language',
    'language_hint'              => 'Controls the display language for the merchant dashboard.',
    'date_format'                => 'Date Format',
    'points_expiration'          => 'Points Expiration Default',
    'points_expiration_hint'     => 'Default setting applied when creating a new Points campaign.',
    'exp_never'                  => 'Never expire',
    'exp_recommended'            => 'Recommended',
    'exp_months'                 => 'Expire after N months',
    'exp_years'                  => 'Expire after N years',
    'exp_duration_label'         => 'Duration (:unit)',
    'birthday_default'           => 'Birthday Reward Default',
    'birthday_default_hint'      => 'Default setting applied when creating a new Points campaign.',
    'birthday_enabled_label'     => 'Enable birthday bonus by default',
    'merchant_name'              => 'Merchant Name',
    'email_address'              => 'Email Address',
    'account_created'            => 'Account Created',
    'current_plan'               => 'Current Plan',
    'subscription'               => 'Subscription',
    'manage_subscription'        => 'Manage Subscription',
    'last_password_change'       => 'Last Password Change',
    'never_changed'              => 'Never changed',
    'change_password'            => 'Change Password',
    'current_password'           => 'Current Password',
    'new_password'               => 'New Password',
    'confirm_password'           => 'Confirm Password',
    'trial_professional_active'  => '(Professional features active)',
    'trial_free_limits'          => '(Free plan limits apply)',
    'account_info'               => 'Account details are managed by OneMember. To update your email address, please contact support.',
    'password_updated'              => 'Password changed successfully.',
    'email_notifications'           => 'Email Notifications',
    'email_notifications_hint'      => 'Choose which emails you receive. Billing and security alerts cannot be disabled.',
    'email_product_updates'         => 'Product updates and new feature announcements',
    'email_tips'                    => 'Tips and best practices',
    'email_feature_announcements'   => 'Feature announcements and newsletters',
    'email_always_on_note'          => 'Billing receipts and security alerts are always sent.',
    // Branding
    'branding_title'               => 'Branding',
    'branding_badge'               => 'White Label Lite',
    'branding_powered_by_note'     => 'Your logo will appear alongside "Powered by OneMember" — your brand, our platform.',
    'logo_label'                   => 'Business Logo',
    'logo_current'                 => 'Current logo',
    'logo_current_none'            => 'No logo uploaded yet — your sidebar shows your initials until you add one.',
    'logo_recommended'             => '400 × 400 px or larger, square or landscape',
    'brand_color'                  => 'Primary Brand Color',
    'brand_color_hint'             => 'Used for buttons and accents in emails.',
    'secondary_color'              => 'Secondary Brand Color',
    'secondary_color_hint'         => 'Used for backgrounds and secondary elements.',
    'business_tagline'             => 'Business Tagline',
    'business_tagline_ph'          => 'e.g. Earn more. Save more. Love more.',
    'business_tagline_hint'        => 'Short motto shown in your member-facing pages. Max 100 characters.',
    'receipt_footer'               => 'Receipt / Email Footer',
    'receipt_footer_ph'            => 'e.g. Thank you for being a valued member! Contact us at hello@yourbusiness.com.',
    'receipt_footer_hint'          => 'Appears at the bottom of billing receipts and subscription emails. Max 500 characters.',
    'online_presence'              => 'Online Presence',
    'facebook_url'                 => 'Facebook Page',
    'instagram_url'                => 'Instagram Profile',
    'line_url'                     => 'LINE Official Account',
    'save_profile'                 => 'Save Profile',
    'cancel'                       => 'Cancel',
    'winback_days'      => 'Win-back Alert (days)',
    'winback_days_hint' => 'Email me when a member has not visited for this many days. Set 0 to switch off.',

    // BETA-008B — Localization / Global Settings
    'tab_localization' => 'Localization',
    'globals_moved_note' => 'Country, currency, timezone and language now live in the Localization tab:',
    'localization_intro' => 'Where you operate, what you charge in, and which languages your team and your customers see. These settings apply across OneMember.',
    'localization_updated' => 'Localization settings updated.',
    'primary_currency' => 'Primary currency',
    'accepted_currencies' => 'Additional accepted currencies',
    'accepted_currencies_hint' => 'Currencies you accept alongside your primary currency. Shown to customers for information only.',
    'currency_conversion_future' => 'Automatic currency conversion is planned as future work — prices are never converted by OneMember.',
    'internal_language' => 'Internal language',
    'internal_language_hint' => 'The language your team sees in the merchant dashboard.',
    'customer_languages' => 'Customer-facing languages',
    'customer_languages_hint' => 'Languages offered on your storefront, member portal and join pages. Customers can switch between them; the first selected is the default.',
    'customer_languages_fallback_note' => 'Languages without full translations yet are shown in English until translations are available.',
    'localization_examples_title' => 'Examples',
    'localization_example_th' => 'A Bangkok cafe: internal language Thai, customer languages Thai + English, currency THB.',
    'localization_example_kh' => 'A Phnom Penh shop: internal language English, customer languages Khmer + English, currencies KHR + USD.',
];
