...
| **Active Unmerged Branch** | `release-001-beta-readiness` (stack: 001A identity → 001B addresses → 001C wallet → RELEASE-001 audit fixes, off main `20084eb`) — plus `website-002a-public-site` (WEBSITE-002A, DECISION-099) on its own branch. 949 tests green. NOT merged, NOT pushed; beta requires integrating BOTH branches into main (documented in the RELEASE-001 report). |
