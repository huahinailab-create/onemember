...
| **Active Unmerged Branch** | `customer-001a-identity-foundation` (off main `20084eb`) — CUSTOMER-001A customer identity foundation: customers table becomes authenticatable (phone/email + OTP/password), new `customer` guard, OtpService + SmsProvider seam (LogSmsProvider only), E.164 TH/MM normalization, profile + settings with OTP re-verification, DECISION-100 / ADR-016. 899 tests green. Awaiting CTO review; NOT merged, NOT pushed. Separately, `website-002a-public-site` (WEBSITE-002A public site, DECISION-099) also awaits review. |
