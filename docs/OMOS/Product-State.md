...
| **Last Code Commit (main)** | audit(ux): MR-004 merchant readiness audit (`merchant-ready-001-mr-001`) — MERCHANT-READY-001 declared COMPLETE by CTO 2026-07-10. Later commits are docs-only strategy: INTERNATIONAL-001 blueprint (approved), GO-TO-MARKET-001 acquisition strategy (approved) in [Roadmaps/](./Roadmaps/), SALES-001 sales operating system (approved; 10 docs in [Sales/](./Sales/)), and WEBSITE-001 public-website master blueprint (13 docs in [Website/](./Website/)) — the final documentation assignment before pilot merchant acquisition |
| **Last Code Sprint (main)** | **MERCHANT-READY-001 / MR-004 — Merchant Readiness Audit**: senior-QA/UX pass over the whole merchant experience (no features/logic/architecture/schema). Small safe fixes: last 11 raw-English flashes + limit errors localized to existing EN/TH `messages.*` keys, 4 accessibility label fixes (reward search button/input, earn-method select, media-upload native input), hardcoded alt localized, duplicate lang key removed. Verified clean: zero horizontal overflow at 375/768 across ~20 merchant pages, EN↔TH lang parity 100%, consistent localized page titles, no dead links/TODOs. Thailand READY; Myanmar partially ready (customer-side yes, merchant UI/Zawgyi/MMK decimals documented) — full findings in the sprint file. Before that on this branch (all CTO approved): **MR-003** guided launch journey, **MR-002** empty states + contextual help, **MR-001** Merchant Launch Dashboard. Earlier: `c698cea` integration merge. |
| **Active Branch (unmerged)** | `website-002a-public-site` — **WEBSITE-002A: Public Marketing Website MVP**. Implements the approved WEBSITE-001 blueprint against the existing corporate site: repositioned Home/Features/Industries/Pricing/About/FAQ/Contact/Resources to "Relationships Matter" merchant-growth framing; **fixed fabricated testimonials and an inconsistent setup-time claim found live in `home.blade.php`** (testimonials now ship hidden until real); Industries rebuilt to the exact 10 blueprint segments; FAQ expanded to 34 questions with `FAQPage` schema; Contact rebuilt LINE-first with an honest response-time promise; Pricing kept its already-correct `TBA`/`Custom` DECISION-014 placeholders and gained `(planned)` labels for unshipped Enterprise features; new `services.line.oa_url` config gates all LINE CTAs (unset — no LINE ID exists, none invented); fixed a real Blade `@context`-directive JSON-LD corruption bug and a real 375px horizontal-overflow bug. **Polish pass (2026-07-15):** slim Bootstrap-only JS bundle for marketing pages (−55% JS, no Alpine/Cropper), PNG og-image (SVG renders blank on LINE/FB/Twitter), `/sitemap.xml` route (robots.txt promised it; 404'd since RELEASE-1B), skip link + `<main>` landmark, h2→h3 accordion headings, `prefers-reduced-motion`, 44px mobile touch targets, hero-mockup trust fix (no statistic-like figures, aria-hidden), "Start Free" CTA per blueprint, 22 inline pink styles → `text-pink` utility. 886 tests green, build clean. Awaiting CTO review before merge — see DECISION-099. |
