...
| **Last Code Commit** | feat(ux): MR-003 guided launch journey (`merchant-ready-001-mr-001`) |
| **Last Code Sprint** | **MERCHANT-READY-001 / MR-003 — Merchant Onboarding Experience**: guided launch journey — completing any launch step shows why it matters + the ONE deterministic next action (`x-launch.step-success`, fed by `launch_step` flashes from the five step-completing actions); raw-English create flashes localized via existing `messages.*` keys; encouraging steps-left copy on the checklist; calm 🎉 Launch Ready celebration with quick actions at 100%; onboarding finish hands off to the dashboard launch plan (wizard rules untouched). Before that on this branch (both CTO approved): **MR-002** empty states + contextual help on all 8 screens, no-dead-help-links regression test; **MR-001** Merchant Launch Dashboard (tenant-scoped checklist, deterministic next action, health card). Earlier: `c698cea` integration merge (help manual, OMEGA-001A–E, PLATFORM-002; `GdImagePipeline` bound). |
