...
| **Integration Branch (unmerged)** | `integration-001-beta-rc` — unified beta release candidate merging `website-002a-public-site` (WEBSITE-002A + polish, DECISION-099) and `release-001-beta-readiness` (CUSTOMER-001A identity → 001B addresses → 001C wallet → RELEASE-001 audit fixes; DECISION-100/101/102). All source branches preserved. NOT merged to main, NOT pushed. |
