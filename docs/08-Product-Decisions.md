...
## DECISION-097 — OMEGA-001A Frontend: Reusable Premium Image Upload

- **Date:** 2026-07-08
- **Status:** Approved
- **Decision:** Per explicit Product Owner/CTO direction, this is an approved new feature (not a bug fix): the Product Image form control gets a premium drag/drop + crop + rotate experience, built as a reusable component so future modules (merchant logo, staff avatar, supplier logo, gallery items, documents) can adopt it without rebuilding it. `Cropper.js` (`^1.6.2`) is added as a project dependency — the classic stable `new Cropper(image, options)` API, not the v2 web-component rewrite. `resources/js/product-image.js` enhances every `[data-media-upload]` root on the page (registered from `resources/js/app.js`, the existing single Vite entry — no new entry point needed). The Blade half is `<x-ui.media-upload>`, a generic anonymous component with no Product-specific logic (name/aspect/presets/remove-name are all props). Cropping happens client-side only: `Cropper.getCroppedCanvas().toBlob()` produces the file that actually gets uploaded, swapped into the real `<input type="file">` via `DataTransfer` right before the form submits — the server (`MediaService`, `ProductController`) is unmodified and simply receives cropped bytes instead of the original. Progressive enhancement is structural, not a class toggle on `<body>`: the plain `<input type="file">` (and, in edit mode, its "remove" checkbox) live in `.media-upload-native-fallback`, visible and fully functional by default; `product-image.js` only hides that fallback and reveals `.media-upload-enhanced` after it successfully wires up a root, inside a try/catch — so a JS error leaves the native controls exactly as they were.
- **Reason:** The ticket that triggered this work described the drag/drop + Cropper.js UI as already built and merely "broken," but no such file, dependency, or UI pattern existed anywhere in the repository before this decision (confirmed by search across `resources/`, `package.json`, and `docs/` before writing any code). Rather than guess, the mismatch was raised to the Product Owner/CTO, who approved it as new work. Building it as a generic component (not hardcoded to Products) avoids the same duplication problem OMEGA-001C was written to prevent on the backend.
- **Impact:** New: `resources/js/product-image.js`, `resources/views/components/ui/media-upload.blade.php`, `lang/en/media.php`, `lang/th/media.php`, `docs/OMOS/12-ADR/ADR-014-Reusable-Media-Upload-UI.md`. Modified: `resources/js/app.js` (imports `product-image.js` + `cropperjs/dist/cropper.css`), `resources/css/app.css` (`.media-upload-*` styles), `resources/views/commerce/products/form.blade.php` (now renders `<x-ui.media-upload>` instead of its own inline file input + preview script), `package.json`/`package-lock.json` (adds `cropperjs@^1.6.2`). No backend, database, or route change; `ProductImageTest` passes unmodified.

---

## DECISION-098 — OMEGA-001E: Store Identity & Public URL Foundation

- **Date:** 2026-07-08
- **Status:** Approved
- **Decision:** Formalize **Business Name** and **Store URL** as two distinct, independently-managed identities on `Merchant` — they always were two different columns (`name` and `slug`) but the platform never treated them as separate *concepts* with separate rules. **Business Name** (`merchants.name`) is the merchant's brand: stored and displayed exactly as typed, forever — no automatic case change, no punctuation stripping (`Merchant::displayName()` from OMEGA-001D is presentation-only and unrelated to this column). **Store URL** (`merchants.slug`, reusing the existing column — no migration) is a technical, ASCII-safe path segment merchants can see and, for the first time, edit in Settings, labeled "Store URL" everywhere in merchant-facing UI (never "slug"). A new `App\Services\StoreIdentity\StoreIdentityService` is the single place that generates (`uniqueSlugFor()`), validates (`isReserved()`, `isAvailable()`), and resolves (`publicStoreUrl()`) a Store URL; `Merchant::booted()`'s creating hook now delegates to it instead of hand-rolling the algorithm inline. Reserved words (route-colliding or confusing segments — `admin`, `store`, `settings`, etc.) are documented in one place, `config/store_identity.php`, and enforced both at auto-generation time and on manual edit. Editing is validated server-side (format regex, uniqueness, reserved-word check) and enhanced client-side (live sanitize-as-you-type, a debounced live-availability check against a new read-only `GET /settings/store-url/availability` endpoint, a copyable public-URL preview) but changing it only ever warns via `confirm()` that old marketing links/QR codes will stop working — **no redirect mechanism was built**; that's explicitly out of scope for this sprint per spec.
- **Reason:** A merchant should be free to have a brand name with any casing, punctuation, or non-Latin script (this is exactly why OMEGA-001D added presentation-only title-casing rather than mutating the stored name) while still having a clean, safe, unique URL segment for their public storefront — the two have fundamentally different constraints and conflating them (e.g. showing "slug" as a technical term, or forcing URL-safe rules onto the brand name field) is confusing for a non-technical small-business owner. Centralizing generation/validation into `StoreIdentityService` (rather than duplicating the collision/reserved-word logic anywhere a future module needs a merchant's public path) follows the same reuse discipline established by `MediaService` (ADR-013).
- **Impact:** New: `app/Services/StoreIdentity/StoreIdentityService.php`, `config/store_identity.php`, `GET /settings/store-url/availability` route + `SettingsController::checkStoreUrlAvailability()`, `docs/OMOS/12-ADR/ADR-015-Store-Identity-and-Public-URL.md`, `StoreIdentityTest` (15 tests). Modified: `Merchant::booted()` (delegates slug generation to the service — same algorithm, same output, existing merchants' slugs untouched), `UpdateMerchantProfileRequest` (new optional `slug` field: format/uniqueness/reserved-word validation), `SettingsController::updateProfile()` (blank Store URL = leave unchanged), `resources/views/settings/index.blade.php` (Store URL field: prefix, live sanitize/availability JS, public-URL preview + copy button, confirm-before-submit warning on change), `lang/en+th/settings.php`. No database migration (reuses `merchants.slug`), no existing route changed, no redirect/backward-compat mechanism for changed URLs (explicitly deferred).

---

## DECISION-099 — WEBSITE-002A: Public Marketing Website MVP Implements the Existing Corporate Site

- **Date:** 2026-07-14
- **Status:** Approved (implementation on unmerged branch `website-002a-public-site`, awaiting CTO review before merge)
- **Decision:** WEBSITE-002A implements the approved WEBSITE-001 blueprint by repositioning the **existing** onemember.co corporate site (`CorporateController`, `resources/views/corporate/*`, `layouts/corporate.blade.php`, `lang/{en,th}/corporate.php`) rather than building a second, parallel site — that infrastructure (routes, SEO scaffold, i18n) already existed from earlier RELEASE-1B/2A sprints and was structurally close to what the blueprint specifies. Content across Home, Features, Industries, Pricing, About, FAQ, Contact, and Resources (Knowledge Center entry point) was rewritten to the approved "Relationships Matter — your regulars, coming back more often" merchant-growth positioning, replacing prior generic "loyalty platform" language. Two real defects were found and fixed in already-live code during this pass: (1) the home page unconditionally rendered three fabricated testimonials (fake shop names, quotes, and a "40% more often" statistic) — now gated behind `!empty()`-safe `is_array()` check on `corporate.home_testimonials`, shipping hidden until real Founding Merchant quotes exist; (2) a hero stat claimed "2 min" setup time, inconsistent with the approved "10 minutes" proof spine — corrected. Industries page rebuilt to the exact 10 blueprint segments (previously 8, several mismatched — e.g. "Car Wash" instead of "Nail Salons"). Pricing was already DECISION-014-safe (`TBA`/`Custom`, never a number) — extended with `(planned)` labels on unshipped Enterprise features (white-label, multi-branch dashboard, corporate controls) per the assignment's explicit instruction not to sell what doesn't exist. Contact rebuilt LINE-first (six doors: Sales/Support/Partnerships/Media/Investors) with an honest "usually within 2 business hours" promise, replacing an unfulfillable "we reply within 1 business day" claim on a contact form that had no backend handler — the form now builds a client-side `mailto:` link instead. New `config('services.line.oa_url')` (env `LINE_OA_URL`) gates every LINE CTA sitewide; no LINE ID exists yet and none was invented — every LINE button/banner is conditional and simply doesn't render until this is configured.
- **Reason:** Building a second site would have duplicated working SEO/i18n/routing infrastructure and created two sources of truth for corporate content. The fabricated-testimonials/setup-time findings are exactly the class of issue this assignment's audit-before-implementation step was designed to catch — WEBSITE-001's own rule ("no fake quotes, ever") was already being violated in production-adjacent code before this sprint touched it.
- **Impact:** New: `tests/Feature/WebsiteMvpTest.php` (26 tests), `services.line` config block. Modified: `resources/views/layouts/corporate.blade.php` (LINE nav/footer CTAs, Organization JSON-LD, mobile-nav `aria-label`), `resources/views/corporate/{home,features,industries,pricing,about,faq,contact,resources}.blade.php`, `lang/{en,th}/corporate.php` (positioning, industries, features, pricing, about, contact, FAQ content), `resources/css/app.css` (`overflow-x:hidden` on `.corp-body` — fixes a pre-existing Bootstrap `.row` negative-margin overflow at 375px, not introduced this sprint), `tests/Feature/{CorporateLocalizationTest,MerchantAcquisitionTest}.php` (3 assertions updated to match intentionally-changed copy). Also fixed during implementation: a literal `'@context'` string in Blade-compiled PHP is mis-parsed as Laravel's `@context` directive (a Laravel 11+ addition for the `Context` facade), corrupting the JSON-LD structured data on every page that used it — worked around via string concatenation (`'@' . 'context'`) in both the sitewide Organization schema and the FAQ page's `FAQPage` schema. No database migration. No existing route changed. No merge to `main` — this branch awaits CTO review per the assignment.

---

## DECISION-100 — CUSTOMER-001A: Customer Identity Foundation

*(DECISION-099 is allocated to WEBSITE-002A on branch `website-002a-public-site`; this entry deliberately skips it to avoid a merge collision.)*

- **Date:** 2026-07-15
- **Status:** Approved (implementation on unmerged branch `customer-001a-identity-foundation`, awaiting CTO review before merge)
- **Decision:** Build the customer authentication and profile foundation — the beginning of the OneMember Wallet — by making the EXISTING PH2-001A Customer record (ADR-010) authenticatable rather than creating a parallel identity. One person, one identity, many merchants: a customer who joined via scan-to-join and one who registers an account are the same row. New `customer` session guard (merchant `web` guard untouched); sign-in with phone OR email (customer's choice), via OTP or password (customer's choice, same form); passwords optional (OTP-only accounts supported); registration with either identifier, verified by OTP before first sign-in completes. OTP engine: bcrypt-hashed 6-digit codes, 5-min expiry, 5-attempt kill, single-use, supersession, per-destination cooldown + hourly cap, config-driven SmsProvider seam whose only implementation is the development LogSmsProvider (no production SMS integration, no fake production sending); email OTPs via markdown mailable. Contact changes (email/phone) always re-verify: the code goes to the NEW destination and the change applies only on verification (the OTP row's destination IS the pending value). E.164 phone normalization for TH/MM via config country rules. Future federated providers (Apple/Google/LINE/Facebook/SSO) are architecture-only: an IdentityProvider contract documents the seam; the customer_identities link table is deferred until the first real provider ships. Login-side responses never reveal account existence; registration's uniqueness validation is the accepted, rate-limited exception. Deliberate schema relaxations (documented in the migration): customers.phone nullable (email-only accounts), customers.email unique (now a login identifier — production must pre-check duplicates). Explicitly NOT built per charter: payments, delivery routing, GPS, wallet UI.
- **Impact:** New: `customer` guard/provider (config/auth.php), config/customer_identity.php, 2 migrations (customers auth fields; customer_otps), CustomerOtp model, app/Services/CustomerIdentity/{OtpService, PhoneNumberService, LogSmsProvider, Contracts/{SmsProvider, IdentityProvider}}, CustomerOtpMail + view, 5 controllers under Http/Controllers/Customer, 21 routes under /account (app domain), customer layout + 8 views, lang/{en,th}/customer.php, 45 tests, ADR-016. Modified: Customer model (extends Authenticatable; all ADR-010 behaviour preserved), CustomerFactory, AppServiceProvider (SMS binding + 4 rate limiters), bootstrap/app.php (guard-aware guest/user redirects), SecurityEventSubscriber (fixed: assumed every Login event carried a merchant User with non-null email — phone-only customer logins crashed it; merchant logging unchanged). No merchant auth changes; guest checkout/storefront/portal remain public.

---

## DECISION-101 — CUSTOMER-001B: Customer Address Book & Checkout Address Foundation

- **Date:** 2026-07-15
- **Status:** Approved (implementation on unmerged branch `customer-001b-saved-addresses`, stacked on `customer-001a-identity-foundation` per CTO instruction; both reviewed together before merge)
- **Decision:** Build the customer's permanent, customer-owned address book — durable identity infrastructure for every future fulfillment surface (food delivery, retail shipping, service appointments, hotel delivery, OneMember Wallet), not a delivery form. Schema stores administrative areas generically (admin_area_1 largest → admin_area_4 smallest); which fields a country uses, their local names (TH province/district/subdistrict vs MM state-region/district/township/ward-village), required fields, and postcode format live in config/customer_address.php — a new country is one config entry, no migration. Unlimited addresses (sanity cap 100), custom labels with suggestions (Home/Work/Office/Parents/Hotel/Other), exactly one default among active addresses enforced by AddressBookService (first address auto-default; deleting/archiving the default promotes the most recently used active address); archive preserves history; delete is soft; duplicate never copies default; every write trims and normalizes the contact phone to E.164. Checkout: signed-in customers pick from "Deliver to" (default first) or add a new country-aware address with an optional save-to-book checkbox; guests keep the existing free-text field untouched. MERCHANT PRIVACY BY CONSTRUCTION: the order stores only a plain-text snapshot of the chosen address in the existing orders.address column — no FK, no uuid, no orders schema change — so merchants can never traverse into the book and later book edits never rewrite a received order. Server enforces ownership + active status on every chosen address; foreign addresses 404 / fail validation without confirming existence. Latitude/longitude columns exist, nullable, never required. Explicitly NOT built per charter: GPS, maps, delivery routing, delivery pricing, wallet UI.
- **Impact:** New: config/customer_address.php, customer_addresses migration, CustomerAddress model (country-aware displayLines/orderSnapshot), AddressBookService (invariants + country-aware validation rules), AddressController (10 routes under /account/addresses, auth:customer only), address book views + shared _fields partial, lang/{en,th}/customer_address.php, CustomerAddressFactory, 32 tests, ADR-017. Modified: Customer model (addresses relation), StorefrontController + storefront show view (address selector for signed-in customers; guest path unchanged), PublicOrderController (resolveCustomerAddress snapshot logic), CSS. Test hygiene: FakeSmsProvider extracted to tests/Support (CUSTOMER-001A test files now pass standalone).

---

## DECISION-102 — CUSTOMER-001C: OneMember Wallet MVP

- **Date:** 2026-07-15
- **Status:** Approved (implementation on unmerged branch `customer-001c-wallet-mvp`, stacked on CUSTOMER-001B; the 001A/001B/001C stack is reviewed together before merging into main)
- **Decision:** Ship the first OneMember Wallet as the customer's home in the ecosystem — a long-term relationship hub ("my relationships with local businesses live here"), not a points viewer. Architecture: a READ MODEL (App\Services\Wallet\WalletService) aggregating exclusively over the customer's own consented CustomerMemberLinks — memberships, home summary, rewards grouped by merchant, chronological activity (joins, all five transaction types, orders), and order history; cross-customer/cross-merchant leakage impossible by construction; future wallet features (gift cards, subscriptions, appointments, membership cards, notifications, AI recommendations) are new aggregate methods + nav items, no redesign. Surfaces: wallet home (welcome by first name, merchants-joined + rewards-available stats, previews, quick links), My Places, read-only membership detail (balance labelled points OR stamps per the merchant's programme — balances NEVER combined across merchants), My Rewards with honest statuses only (Available / Coming soon; the domain has no reward expiry so none is invented) and a visibly disabled Redeem button, Activity, My Orders (items, total, status, delivery-address snapshot used, Order-again link to the storefront). Login/OTP/password-reset all land on the wallet. Two additive columns: orders.customer_id (orders placed signed-in belong to the wallet; guests stay NULL) and customers.preferences JSON (communication channel + marketing consent now, notification settings later without migration). Six reserved Coming Soon tiles honestly labelled. Explicitly NOT built per charter: payments, gift cards, subscriptions, bookings, marketplace, GPS, notifications, native app, redemption.
- **Impact:** New: WalletService, WalletController (6 routes under /account/wallet), 6 wallet views + 4 partials, lang/{en,th}/customer_wallet.php, wallet CSS (pill nav, stat cards, membership cards, tiles; reduced-motion), migration (preferences + orders.customer_id), 18 tests, ADR-018. Modified: customer layout (wallet nav slot, wider shell, profile shortcut), settings view (preferences section + notifications placeholder), AccountController (updatePreferences), AuthController/PasswordResetController/bootstrap redirects (land on wallet), PublicOrderController (sets customer_id when signed in), Customer/Order models. CUSTOMER-001A landing-assertions updated accordingly.
